...
class CompleteCare extends StatelessWidget {
  CompleteCare({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title:
            const Text('Complete Care', style: TextStyle(color: Colors.white)),
        centerTitle: true,
...
class ListViewDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      stream: firestoreService.getCompleteCare(),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          List completeCareList = snapshot.data!.docs;
          return ListView.builder(
            itemCount: completeCareList.length, // Ensure itemCount is set
            itemBuilder: (context, index) {
              DocumentSnapshot documents = completeCareList[index];
        ...
        if (!stateManager.isLoading && stateManager.errorMessage == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              ...
              content: Text('Complete Care submitted successfully!'),
            ),
          );
        }
      },
